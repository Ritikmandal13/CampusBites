...
export function useMyOrders() {
  return useQuery({
    ...
    queryFn: async (): Promise<Order[]> => {
      ...
      if (!session?.user) {
        console.log('🔍 useMyOrders: No user session')
        return []
      }
      ...
      try {
        ...
        const ordersUrl = `${supabaseUrl}/rest/v1/orders?user_id=eq.${userId}&select=*,order_items(*)&order=created_at.desc`
        ...
        if (!response.ok) {
          const errorText = await response.text()
          console.error('❌ useMyOrders: HTTP error:', response.status, errorText)
          throw new Error(`HTTP ${response.status}: ${response.statusText}`)
        }
        
        const data = await response.json()
        console.log('✅ useMyOrders: Loaded', data.length, 'orders')
        
        if (!Array.isArray(data)) {
          console.error('❌ useMyOrders: Expected array but got:', typeof data)
          return []
        }
        ...
        if (err.name === 'AbortError') {
          console.error('❌ useMyOrders: Request aborted (timeout)')
          throw new Error('Request timed out - check network connectivity')
        }
    ...
    staleTime: 5000, // 5 seconds - orders update frequently
    retry: 1,
  })
}
